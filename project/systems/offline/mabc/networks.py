...
def make_default_networks(
    ...
    behaviour_networks_layer_sizes: Union[Dict[str, Sequence], Sequence] = None,
    ...
    if architecture_type == ArchitectureType.feedforward:
        if not value_networks_layer_sizes:
            ...
            behaviour_networks_layer_sizes = value_networks_layer_sizes
        value_network_func = snt.Sequential
        behaviour_network_func = value_network_func
    ...
    if isinstance(behaviour_networks_layer_sizes, Sequence):
        behaviour_networks_layer_sizes = {
            key: behaviour_networks_layer_sizes for key in specs.keys()
        }
    ...
    for key, spec in specs.items():
        ...
        if architecture_type == ArchitectureType.feedforward:
            ...
            behaviour_network = [
                networks.LayerNormMLP(
                    behaviour_networks_layer_sizes[key][:-1],
                    activate_final=True,
                    seed=seed,
                ),
                snt.GRU(behaviour_networks_layer_sizes[key][-1]),
            ]
        ...
        behaviour_network = behaviour_network_func(behaviour_network)

        observation_networks[key] = observation_network
        value_networks[key] = value_network
        behaviour_networks[key] = behaviour_network
        action_selectors[key] = EpsilonGreedy

    return {
        "values": value_networks,
        "behaviours": behaviour_networks,
