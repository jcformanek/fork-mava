...
class MABCQExecutor(DQNExecutor):
    ...
    def _policy(
        ...
        value_state: types.NestedTensor,
        ...
        action_values, new_value_state = self._value_networks[agent_key](embed, value_state)

        # Behaviour 
        logits, new_behaviour_state = self._behaviour_networks[agent_key](embed, behaviour_state)
        imt = tf.nn.softmax(logits)
        imt = imt  > self._bcq_threshold
        agent_legal_actions = tf.cast(batched_legal_actions, "bool")
        imt = tf.math.logical_and(imt, agent_legal_actions)
        legal_imt_actions = tf.cast(tf.reduce_max(tf.cast(imt, "int64"), axis=-1, keepdims=True), 'bool')
        selector_mask = tf.where(legal_imt_actions, imt, agent_legal_actions)

        # Constrain actions
        action_values = tf.where(selector_mask, action_values, -999999999)

        # Pass action values through action selector
        action = self._action_selectors[agent](action_values, batched_legal_actions)

        return action, new_value_state, new_behaviour_state

    @tf.function
    def _select_actions(
        self,
        observations: Dict[str, types.NestedArray],
        value_states: Dict[str, types.NestedArray],
        behaviour_states: Dict[str, types.NestedArray],
    ) -> types.NestedArray:
        """The part of select_action that we can do inside tf.function"""
        actions: Dict = {}
        new_value_states: Dict = {}
        new_behaviour_states: Dict = {}
        for agent, observation in observations.items():
            actions[agent], new_value_states[agent], new_behaviour_states[agent] = self._policy(
                agent,
        ...
        return actions, new_value_states, new_behaviour_states
    ...
    def select_actions(
        ...
        actions, new_value_states, new_behaviour_states = self._select_actions(observations, self._value_states, self._behaviour_states)

        # Convert actions to numpy arrays
        actions = tree.map_structure(tf2_utils.to_numpy_squeeze, actions)

        # Update agent core state
        for agent, state in new_value_states.items():
            self._value_states[agent] = state
            self._behaviour_states[agent] = new_behaviour_states[agent]
    ...
    def observe_first(
        ...
        for agent, _ in timestep.observation.items():
            # index network either on agent type or on agent id
            agent_key = self._agent_net_keys[agent]
            self._value_states[agent] = self._value_networks[agent_key].initial_state(1)
