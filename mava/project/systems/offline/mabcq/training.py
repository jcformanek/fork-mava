...
class MABCQTrainer:
    ...
    def __init__(
        ...
        for net_key in self.unique_net_keys:
            ...
            self._system_network_variables["behaviours"][
                net_key
            ] = self._behaviour_networks[net_key].variables
    ...
    def _forward(self, inputs: reverb.ReplaySample) -> None:
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            for agent in self._agents:
                ...
                logits = logits[1:] # chop off first timestep
                imt = tf.nn.softmax(logits)
                imt = imt  > self._bcq_threshold
                agent_legal_actions = tf.cast(legal_actions[agent][1:], "bool")
                ...
                # Constrain actions
                q_t_selector = tf.where(selector_mask, q_t_selector, -999999999)
                ...
                logits, _ = train_utils.combine_dim(
                    logits
                )
                ...
                behaviour_loss = self._behaviour_loss_fn(a_tm1, logits)

                # Combined loss
                combined_loss = value_loss + behaviour_loss + 1e-2 * tf.reduce_mean(logits ** 2)
                ...
                masked_loss = combined_loss * zero_padding_mask
