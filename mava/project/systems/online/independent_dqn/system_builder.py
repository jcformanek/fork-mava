...
class IndependentDQN:
    ...
    def run_single_proc_system(self, training_steps_per_episode = 1, evaluator_period=5, max_executor_steps=1_000_000):
        ...
        episode = 0
        executor_steps = 0
        while executor_steps < max_executor_steps:
            ...
            if episode % evaluator_period == 0 and self._wandb and episode >= self._min_replay_size:
                # Add all logs
                all_logs = {}
                # Add executor logs
                for key, stat in executor_stats.items():
                    all_logs["executor_"+key] = stat
                # Add evaluator logs
    ...
    def _extra_trainer_setup(self, trainer):
        """Pass lambda to trainer."""
        trainer.extra_setup(lambda_=self._lambda)
        return trainer